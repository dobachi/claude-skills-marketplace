...
| **通ったのに根拠が増えていない** | `no_regression` の `※ 根拠の実体は増えていない` を読む。形式（出典参照・出典表）だけ増えて実体（引用行・引用スパン）が 0→0 なら、追跡可能性が整っただけ。実測3回 |
| **候補の大半が偽陽性** | 出典エントリが本文に散在する書式では、見出しの年号が「数字を含む断定」として発火する。実測 8/8件・28/52件。**書式を固定すれば構造的に起きない**（`references/document-format.md`） |
| **`【推】` や節名で対象外にされた** | 編集点が抜け道になっている。`SKIP_MARK` / `SKIP_SECTIONS` は**塞げない**。通ったあとに差分を読むしかない |
