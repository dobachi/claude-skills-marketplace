...
| `ALLOWED` を上げるか迷う | 上げてよい。ただし**上げた数だけ人が確認済みであること**が前提。**緩めた事実と中身は、検出器の定数だけでなく成果物側にも書く**（読む人に「機械が見ていない範囲」が伝わらないため） |
| 候補が「本報告書の §X は…」ばかり | **自己言及の偽陽性。** 検出器は外部出典を引く文書を前提にしており、文書が自分を指す節（要約・提言・限界）を想定していない。節番号の数字に反応する。実運用で16件中12件がこれだった |
