...
1. **Coordinates computed once from a shared grid, never hand-placed.** `build_deck.py` builds one `grid` dict (`make_grid`) in inches and every renderer reads from it. The accent rule (a short ~1 in × ~0.045 in rect) is drawn at `(grid.marginX, grid.ruleY[family])` — the same inputs produce the same EMU coordinates on every page of a family, so the mark is pixel-identical and cannot drift. Drift only happens when a shape is nudged by hand; nothing here is. `rule` in the theme / `meta`: `true` draws it only on the pages that mark a turn (`title` / `section` / `statement`); `"all"` also draws it under content titles; `false` drops it.

2. **A short rule anchored to the left margin**, not a full-width fill. It is a *short* mark computed from the one grid, so it never spans edge-to-edge and never depends on title length. It is also **not on every page**: a mark that repeats on every slide is wallpaper, and "a line and a bold sentence" on every opener, divider and verdict is its own generated-deck signature. The rule is punctuation for the turns; the four large-type pages each have their own composition (see `spec-format.md`), and the quote uses a vertical bar instead. (If you want the even stronger "exists once" guarantee, the hairline can be promoted onto a python-pptx slide layout so it is inherited rather than re-emitted — but identical computed coordinates already give zero drift.)
