...
not_applicable_when: "組織のテンプレートが装飾を含む場合。テンプレートを書き換えず、そのまま使う（pptx-constraint-template）。および core-ornament-part-vocabulary の8種の部品。取り除くと意味が減るので装飾ではない"
source:
  kind: house
  key: house
  ref: "本リポジトリの決め"
...
「装飾的な角丸枠」という禁止は、**装飾的な**ものに掛かっている。カード・
ステップの箱・象限は角丸の枠だが、それぞれ「これらは等価である」「順序の
一段である」「2軸上のここにある」を運ぶ。取り除けば意味が減るので、
`core-diagram-information-test` を通る（`core-ornament-part-vocabulary`）。

判定は形ではなく、取り除いたときに何が失われるかである。8種の一覧に載って
いても、そのスライドで何も運んでいなければ装飾であり、ここで禁止される。
