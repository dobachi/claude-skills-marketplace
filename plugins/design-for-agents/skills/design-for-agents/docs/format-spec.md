...
2026-08-29 に 61条件のうち 29 を昇格させた。結果は次のとおり。

| | 件数 |
|---|---|
| statement をそのまま使えた | 26 |
| statement の修正が要った | 3 |

**修正が要った3件は、いずれも書式のせいではなく、条件が事実として
間違っていた**。書式が原因の書き直しは0件だったので、5.1〜5.3 の前提は成立している。

修正の内訳（いずれも実物に当てて初めて分かった）:

- `core-layout-one-grid#left-edges-aligned` — 右揃えの要素は右端で揃う。
  左端だけを見る条件はページ番号を誤検出していた
- `core-color-four-slots#only-four-slots` — 特定の色値との一致を見ていた。
  ブランドで `accent` は差し替わるので、**数えるべきは色数**だった
- `core-emphasis-ladder#one-emphasis-per-slide` — アクセント色の出現回数を
  数えていた。2段組の両列見出しは競合ではなく**反復**なので、
  書式（大きさ・太さ）が違うものが2種類あるかを見るのが正しい

**教訓は「条件は実物に当てるまで正しいと分からない」**である。書式が検出器互換で
あることと、条件が正しいことは別である。
