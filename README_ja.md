...
| **pptx-build** | AIっぽく見えない、白基調でシンプルな.pptxファイルを生成。全要素を共有グリッドに固定し、ズレて見える帯（フル幅の色付き矩形）を一切描かない設計（アクセントはスライドマスターに配置）。ブランドに合わせて差し替え可能なJSテーマモジュール方式。PptxGenJS（Node）ベース、LibreOfficeプレビュー同梱。 |
