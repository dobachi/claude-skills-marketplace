...
| **design-for-agents** | 人間向けに書かれたデザイン知を、**AIが迷わず引ける形**に組み直した参照体。ユースケース別のプレイブック（意思決定資料 / 技術説明 / ピッチ / 既存資料のリファクタ / 表 / グラフ / 構造図 / コーポレートテンプレ準拠）が原子ルールを ID で引き、各ルールは権威の階層（MUST / SHOULD / HOUSE）、出典、そして検出器に落とせる書式の完了条件を持つ。決め打ちの値は W3C DTCG 形式のトークンで供給し、やってはいけないことは具体名で禁止する。ルータであって判断は持たず、実ファイルの生成は `pptx-build` に引き渡す。 |
| **pptx-design** | PowerPoint（.pptx）資料の設計**と作り直し**のガイド。「シンプルかつクリーン」を決め打ちの値として定義したデザインシステム（色は4スロット、タイプスケール1本、グリッド1つ、スライドごとの密度上限）に加え、タイポグラフィ、配色、データビジュアライゼーション、構造図、資料ジャンル、アクセシビリティ、スライドマスタ設計。既存資料（引き継ぎ資料・AI生成資料・急ごしらえの資料）のリファクタリング手順も収録 — 5段階の診断、直す順番、その場で手直しするか作り直すかの判断、そして「主張は変えない」という境界線。助言専用で、実ファイルの生成と既存デッキの抽出は `pptx-build` に引き渡す（手書きの python-pptx は書かない）。 参照すべき外部文献のルーティング表も収録 — 「いま何に詰まっているか」（グラフ選択、コントラスト比の規範、ピッチの構成、グリッド、図の意味論など）から引くべき一次資料へ一意に辿れる形にし、取得して引用できる規格・公開資料と、著者に薦めるだけの書籍を分けてある。 |
| **pptx-build** | AIっぽく見えない、白基調でシンプルな.pptxファイルを生成し、既存デッキの作り直しにも対応。python-pptxベース。デフォルトモードは全要素を共有グリッドに固定し、ズレて見える帯を一切描かない設計。テンプレート流し込みモードは、実際の会社の.pptx/.potxを開いて、そのレイアウトとプレースホルダに書き込み、テンプレートのマスター/テーマ/フォント/ロゴをそのまま継承。リファクタリングモードは `extract_deck.py` で既存の.pptxをspecに読み戻し（タイトル・箇条書き・表・グラフ・図・出典・発表者ノート）、表現できなかったものを報告するので、主張はテキストで直してからプレースホルダに書き戻せる。表・グラフのスライド型を備え、手書きの python-pptx に逃げる必要がない。生成後の .pptx を検査する `audit_pptx.py` を同梱し、内容がプレースホルダに入っていないデッキはエラーにする。LibreOfficeプレビュー同梱。 |
