...
| **pptx-build** | AIっぽく見えない、白基調でシンプルな.pptxファイルを生成し、既存デッキの作り直しにも対応。python-pptxベース。同梱プリセット4種（白基調・明朝の編集調・アース系・暗色）で、構図を変えずに見た目だけを差し替えられる。デフォルトモードは全要素を共有グリッドに固定して帯を描かず、さらに内容の形から選ぶ構図の型（カード／ステップ／主役＋従／マトリクス／非対称分割／宣言）を、余白の基準単位1つ・角丸1種・線幅1種だけで描く。テンプレート流し込みモードは、実際の会社の.pptx/.potxを開いてそのレイアウトとプレースホルダに書き込み、マスター/テーマ/フォント/ロゴを継承。リファクタリングモードは `extract_deck.py` で既存の.pptxをspecに読み戻し（タイトル・箇条書き・表・グラフ・図・構図パーツ・発表者ノート）、表現できなかったものを報告する。`validate_deck.py`（構成・型の条件・論旨の背骨）と `audit_pptx.py`（内容がプレースホルダに入っていないデッキをエラーにする）を同梱。設計の判断は `design-for-agents` が持つ。 |
