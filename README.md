...
| **pptx-build** | Generates clean, white-based .pptx files that don't look AI-made — and rebuilds decks that already exist. Four shipped presets (quiet white, serif editorial, warm, dark) swap the look without touching the composition. Default mode: a grid-anchored layout with no drifting bands, plus composed archetypes (`cards` / `steps` / `lead` / `matrix` / `split` / `statement`) chosen from the content's shape and drawn on one spacing scale with one radius and one line weight. Template-fill mode: opens a real corporate .pptx/.potx and writes into its layouts and placeholders, inheriting the template's master/theme/fonts/logos. Refactor mode: `extract_deck.py` reads an existing deck back into a spec — titles, bullets, tables, charts, figures, composed parts, speaker notes — and reports what it cannot carry. Ships `validate_deck.py` and `audit_pptx.py`, which fails any deck whose content is not in layout placeholders. LibreOffice preview. Design judgement lives in `design-for-agents`. |
