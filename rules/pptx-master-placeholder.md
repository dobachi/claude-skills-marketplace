...
テキストボックスはスライドマスターの支配を受けない。マスターを直しても
反映されず、書式の変更は全スライドを手で直す作業になる。
`core-type-one-scale` と `core-layout-one-grid` は、**マスターが効いていて
初めて成立する**。
...
落ちる。これはマスターが何も支配していない状態であり、本ルールの正反対である。
